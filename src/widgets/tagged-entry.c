...
static guint entry_signals[ENTRY_LAST_SIGNAL] = { 0, };
...
static void
demo_tagged_entry_set_property (GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec)
{
...
static void
demo_tagged_entry_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
demo_tagged_entry_class_init (DemoTaggedEntryClass *klass)
{
  ...
  entry_signals[SIGNAL_TAG_REMOVED] = g_signal_new ("tag-removed", DEMO_TYPE_TAGGED_ENTRY, G_SIGNAL_RUN_FIRST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);
}
...
/*
 *  Free result with g_array_unref, g_array_free. The strings are owned by this widget, and should not be freed by the caller.
 */
GPtrArray*
tagged_entry_get_tags (DemoTaggedEntry* entry)
{
	GPtrArray* array = g_ptr_array_sized_new (4);
	GtkWidget* child = gtk_widget_get_first_child((GtkWidget*)entry);
	if (child) {
		for (; child; child = gtk_widget_get_next_sibling(child)) {
			if (DEMO_IS_TAGGED_ENTRY_TAB(child)) {
				const char* val = demo_tagged_entry_tag_get_label((DemoTaggedEntryTag*)child);
...
static void
tagged_entry_foreach_tag (DemoTaggedEntry* entry, void (*fn)(DemoTaggedEntry*, DemoTaggedEntryTag*, gpointer), gpointer user_data)
{
	GtkWidget* child = gtk_widget_get_first_child((GtkWidget*)entry);
	if (child) {
		for (; child; child = gtk_widget_get_next_sibling(child)) {
			if (DEMO_IS_TAGGED_ENTRY_TAB(child)) {
				fn(entry, (DemoTaggedEntryTag*)child, user_data);
			}
		}
	}
}
...
static bool
tagged_entry_has_tag (DemoTaggedEntry* entry, const char* str)
{
	GtkWidget* child = gtk_widget_get_first_child((GtkWidget*)entry);
	if (child) {
		for (; child; child = gtk_widget_get_next_sibling(child)) {
			if (DEMO_IS_TAGGED_ENTRY_TAB(child)) {
				if (!strcmp(str, demo_tagged_entry_tag_get_label((DemoTaggedEntryTag*)child)))
					return true;
			}
		}
	}
...
void
tagged_entry_set_tags_from_string (DemoTaggedEntry* entry, const char* str)
{
	void closed_cb (DemoTaggedEntryTag* tag, DemoTaggedEntry* entry)
	{
	...
	if (split) {
		{
			...
			void fn (DemoTaggedEntry*, DemoTaggedEntryTag* tag, gpointer list)
			{
				GList** l = list;
				*l = g_list_append(*l, tag);
			}

			GList* buttons = NULL;
			tagged_entry_foreach_tag(entry, fn, &buttons);

			for (GList* l= buttons;l;l=l->next) {
				GtkWidget* button = l->data;
				if (!g_strv_contains((const gchar* const*)split, demo_tagged_entry_tag_get_label((DemoTaggedEntryTag*)button)))
					demo_tagged_entry_remove_tag(entry, button);
			}
		}

		char* word;
		for (int i=0; (word=split[i]); i++) {
			if (strlen(word) && !tagged_entry_has_tag(entry, word)) {
				DemoTaggedEntryTag* tag = demo_tagged_entry_tag_new(word);
				demo_tagged_entry_tag_set_has_close_button((DemoTaggedEntryTag*)tag, true);
				gtk_widget_add_css_class (GTK_WIDGET (tag), "blue");
				demo_tagged_entry_add_tag((DemoTaggedEntry*)entry, (GtkWidget*)tag);
...
void
demo_tagged_entry_insert_tag_after (DemoTaggedEntry *entry, GtkWidget *tag, GtkWidget *sibling)
{
	g_return_if_fail (DEMO_IS_TAGGED_ENTRY (entry));
...
void
demo_tagged_entry_remove_tag (DemoTaggedEntry* entry, GtkWidget* tag)
{
	g_return_if_fail (DEMO_IS_TAGGED_ENTRY (entry));
...
static void
on_released (GtkGestureClick *gesture, int n_press, double x, double y, DemoTaggedEntryTag *tag)
{
...
static void
demo_tagged_entry_tag_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
  DemoTaggedEntryTag *tag = DEMO_TAGGED_ENTRY_TAG (object);
...
static void
demo_tagged_entry_tag_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
  DemoTaggedEntryTag *tag = DEMO_TAGGED_ENTRY_TAG (object);
...
static void
demo_tagged_entry_tag_measure (GtkWidget *widget, GtkOrientation orientation, int for_size, int *minimum, int *natural, int *minimum_baseline, int *natural_baseline)
{
	DemoTaggedEntryTag *tag = DEMO_TAGGED_ENTRY_TAG (widget);

	gtk_widget_measure (tag->box, orientation, for_size, minimum, natural, minimum_baseline, natural_baseline);
}

static void
demo_tagged_entry_tag_size_allocate (GtkWidget* widget, int width, int height, int baseline)
{
	DemoTaggedEntryTag *tag = DEMO_TAGGED_ENTRY_TAG (widget);

	gtk_widget_size_allocate (tag->box, &(GtkAllocation) { 0, 0, width, height }, baseline);
}
...
demo_tagged_entry_tag_class_init (DemoTaggedEntryTagClass *class)
{
	...
	signals[SIGNAL_CLICKED] = g_signal_new ("clicked", DEMO_TYPE_TAGGED_ENTRY_TAG, G_SIGNAL_RUN_FIRST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);
	signals[SIGNAL_BUTTON_CLICKED] = g_signal_new ("button-clicked", DEMO_TYPE_TAGGED_ENTRY_TAG, G_SIGNAL_RUN_FIRST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);

	g_object_class_install_property (object_class, PROP_LABEL, g_param_spec_string ("label", "Label", "Label", NULL, G_PARAM_READWRITE));
	g_object_class_install_property (object_class, PROP_HAS_CLOSE_BUTTON, g_param_spec_boolean ("has-close-button", "Has close button", "Whether this tag has a close button", FALSE, G_PARAM_READWRITE|G_PARAM_EXPLICIT_NOTIFY));
...
void
demo_tagged_entry_tag_set_label (DemoTaggedEntryTag *tag, const char *label)
{
...
void
demo_tagged_entry_tag_set_has_close_button (DemoTaggedEntryTag* tag, bool has_close_button)
{
  g_return_if_fail (DEMO_IS_TAGGED_ENTRY_TAG (tag));
