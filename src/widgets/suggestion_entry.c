...
#define DELAYED_TIMEOUT_ID 1000
...
static gboolean
suggestion_entry_changed_timeout_cb (gpointer user_data)
{
  SuggestionEntry *entry = user_data;
...
static void
reset_timeout (SuggestionEntry *entry)
{
  ...
  entry->delayed_changed_id = g_timeout_add (DELAYED_TIMEOUT_ID,
                                            suggestion_entry_changed_timeout_cb,
                                            entry);
}
...
static void
suggestion_entry_activate (SuggestionEntry* self)
{
                                ...
                                SuggestionEntry *self)
{
  ...
  suggestion_entry_activate(self);
}
                              ...
                              SuggestionEntry       *self)
{
  ...
  if (keyval == GDK_KEY_Return ||
    ...
    {
      gboolean popup = gtk_widget_get_visible(self->popup);
      char* revert = (!popup && self->revert) ? self->revert : NULL;
      char* text = revert
        ? self->revert
      ...
      if (popup || revert)
        {
          ...
          if (revert) g_clear_pointer(&self->revert, g_free);
          return TRUE;
        }
    }
