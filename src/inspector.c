...
void menu_play_stop(GtkWidget* widget, gpointer user_data); //defined in main.c
...
#define BORDERMARGIN (2)
	...
	GtkWidget *slider = app.inspector->slider1 = gtk_hscale_new_with_range(0.0,1.0,1.0/(float)OVERVIEW_WIDTH);
	...
	/* play ctrl buttons */
	GtkWidget* hbox2 = app.inspector->pbctrl = gtk_hbox_new(FALSE, 10);
	gtk_box_pack_start(GTK_BOX(vbox), hbox2, EXPAND_FALSE, FILL_FALSE, 0);

	GtkWidget* pb0 = app.inspector->pbpause = gtk_toggle_button_new_with_label("pause");
	gtk_box_pack_start(GTK_BOX(hbox2), pb0, EXPAND_FALSE, FILL_FALSE, 0);
	g_signal_connect((gpointer)pb0, "toggled", G_CALLBACK(cb_playpause), NULL);

	GtkWidget* pb1 = gtk_button_new_with_label("stop");
	gtk_box_pack_start(GTK_BOX(hbox2), pb1, EXPAND_FALSE, FILL_FALSE, 0);
	...
	gtk_alignment_set_padding(GTK_ALIGNMENT(align10), 0, 0, margin_left-BORDERMARGIN, 0);
  ...
  GtkWidget* align11 = gtk_alignment_new(0.0, 0.5, 0.0, 0.0);
	gtk_alignment_set_padding(GTK_ALIGNMENT(align11), 0, 0, margin_left-BORDERMARGIN, 0);
...
static void cb_pitch_toggled(GtkToggleButton *btn, gpointer user_data) {
	...
	gtk_widget_set_sensitive(app.inspector->slider2, app.enable_effect);
	gtk_widget_set_sensitive(app.inspector->cblnk, app.enable_effect);
	if (!app.enable_effect) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app.inspector->cblnk), false);
}
...
#endif
		gtk_widget_set_sensitive(app.inspector->slider2, true);
#endif
		return FALSE;
	}
	return TRUE;
...
void show_player() {
	...
	if (app.playing_id == app.inspector->row_id) {
		/* show player */
		gtk_widget_show(app.inspector->slider1);
		gtk_widget_show(app.inspector->pbctrl);
#if (defined ENABLE_LADSPA)
		gtk_widget_set_sensitive(app.inspector->cbfx, false);
#ifndef VARISPEED
