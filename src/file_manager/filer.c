...
static void detach(Filer* filer_window);
//static void filer_window_destroyed(GtkWidget    *widget, FilerWindow* filer_window);
//static void update_display(Directory *dir, DirAction action, GPtrArray* items, Filer* filer_window);
static void set_scanning_display(Filer* filer_window, gboolean scanning);
#if 0
...
#endif
static void set_selection_state(Filer *filer_window, gboolean normal);
static void filer_next_thumb(GObject *window, const gchar *path);
static void start_thumb_scanning(Filer* filer_window);
#if 0
static void filer_options_changed(void);
static void drag_end(GtkWidget *widget, GdkDragContext *context, FilerWindow *filer_window);
 ...
 */
static gint
open_filer_window(Filer* filer_window)
{
	PF;
 ...
 */
static void
queue_interesting(Filer* filer_window)
{
	DirItem	*item;
	ViewIter iter;
	...
	{
		if (item->flags & ITEM_FLAG_NEED_RESCAN_QUEUE) dir_queue_recheck(filer_window->directory, item);
	}
}
...
update_display(Directory *dir, DirAction action, GPtrArray* items, Filer* filer_window)
{
	...
	switch (action)
	{
		...
		case DIR_END_SCAN:
			...
			if (filer_window->had_cursor && !view_cursor_visible(view))
			{
				ViewIter start;
			...
			//if (filer_window->auto_select) display_set_autoselect(filer_window, filer_window->auto_select);
...
attach(Filer* filer_window)
{
	...
	dir_attach(filer_window->directory, (DirCallback) update_display, filer_window);
	//filer_set_title(filer_window);
...
static gboolean
may_rescan(FilerWindow *filer_window, gboolean warning)
{
	Directory *dir;
 ...
 */
void
filer_selection_changed(Filer *filer_window, gint time)
{
	g_return_if_fail(filer_window != NULL);
	...
	if (!view_count_selected(filer_window->view)) return;		/* Nothing selected */

	if (filer_window->temp_item_selected == FALSE &&
		gtk_selection_owner_set(GTK_WIDGET(filer_window->window), GDK_SELECTION_PRIMARY, time))
	{
...
gint
filer_key_press_event(GtkWidget	*widget,GdkEventKey *event, Filer* filer_window)
{
	ViewIface *view = filer_window->view;
	ViewIter cursor;
...
filer_change_to(Filer* filer_window, const char *path, const char *from)
{
	...
	if (!new_dir)
	{
		//delayed_error("Directory '%s' is not accessible", sym_path);
	...
	if(filer_window->directory) detach(filer_window);
	g_free(filer_window->real_path);
	g_free(filer_window->sym_path);
	...
	//had_cursor = had_cursor || view_cursor_visible(filer_window->view);
 ...
 */
FilerWindow*
filer_opendir(const char *path, FilerWindow *src_win, const gchar *wm_class)
{
	FilerWindow	*filer_window;
	char		*real_path;
	DisplayStyle    dstyle;
...
static void filer_add_widgets(FilerWindow *filer_window, const gchar *wm_class)
{
	...
	/* And the thumbnail progress bar (also hidden) */
	{
		...
		gtk_box_pack_start(GTK_BOX(filer_window->thumb_bar),
				cancel, FALSE, TRUE, 0);
		g_signal_connect_swapped(cancel, "clicked", G_CALLBACK(filer_cancel_thumbnails), filer_window);
	}
...
static gint
clear_scanning_display(Filer* filer_window)
{
	//if (filer_exists(filer_window)) filer_set_title(filer_window);
	return FALSE;
}
#endif

static void
set_scanning_display(Filer* filer_window, gboolean scanning)
{
	if (scanning == filer_window->scanning) return;
...
static void
set_selection_state(Filer *filer_window, gboolean normal)
{
	GtkStateType old_state = filer_window->selection_state;

	filer_window->selection_state = normal ? GTK_STATE_SELECTED : GTK_STATE_INSENSITIVE;
...
void
filer_cancel_thumbnails(Filer* filer_window)
{
 ...
 */
static gboolean
filer_next_thumb_real(GObject *window)
{
	Filer *filer_window;
	gchar	*path;
	int	done, total;
	...
	filer_window->thumb_queue = g_list_remove(filer_window->thumb_queue, path);
	g_free(path);
 ...
 */
static void
filer_next_thumb(GObject *window, const gchar *path)
{
...
static void
start_thumb_scanning(Filer* filer_window)
{
...
void
filer_create_thumb(Filer* filer_window, const gchar *path)
{
	dbg(0, "%s", path);
	if (g_list_find_custom(filer_window->thumb_queue, path, (GCompareFunc) strcmp))	return;

	if (!filer_window->thumb_queue) filer_window->max_thumbs=0;
	filer_window->max_thumbs++;

	filer_window->thumb_queue = g_list_append(filer_window->thumb_queue, g_strdup(path));

	if (filer_window->scanning) return; /* Will start when scan ends */
...
filer_create_thumbs(Filer* filer_window)
{
	...
	while ((item = iter.next(&iter)))
	{
		 ...
		 */
		if (!found) filer_create_thumb(filer_window, (char*)path);
	}
}

#if 0
static void
filer_options_changed(void)
{
...
gboolean
filer_match_filter(Filer* filer, DirItem *item)
{
	g_return_val_if_fail(item != NULL, FALSE);

	if(is_hidden(filer->real_path, item) &&
	   (!filer->temp_show_hidden && !filer->show_hidden))
		return FALSE;

	switch(filer->filter) {
		case FILER_SHOW_GLOB:
			return fnmatch(filer->filter_string, item->leafname, 0)==0 ||
			  (item->base_type==TYPE_DIRECTORY &&
			   !filer->filter_directories);
