...
void            ayyi_dbus_object_new     (struct _ayyi_action*, const char* name, gboolean from_source, AyyiIdx src_idx, guint32 parent_idx, struct _song_pos* stime, guint64 len, guint32 inset);
void            ayyi_dbus_part_new       (struct _ayyi_action*, const char* name, gboolean from_source, AyyiIdx src_idx, guint32 parent_idx, struct _song_pos* stime, guint64 len, guint32 inset);
void            ayyi_dbus_object_del     (struct _ayyi_action*, int object_type, uint32_t object_idx);
...
DBusGProxyCall* dbus_set_prop_string(struct _ayyi_action*, int object_type, int property_type, AyyiIdx, const char* val);
