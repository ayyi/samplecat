...
ts_timelog (const char *tag)
{
	...
	if (tag == ts_timing_report) {
		double diff = (now.tv_sec - t0.tv_sec) + (now.tv_usec - t0.tv_usec) * 0.000001;
		double pc = 0.0;
		...
		double diff = (now.tv_sec - base.tv_sec) + (now.tv_usec - base.tv_usec) * 0.000001;
...
void
ts_timing_add (const char *tag, const double diff, const int cnt)
{
