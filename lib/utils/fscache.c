...
static gboolean      purge_hash_entry(gpointer key, gpointer data, gpointer user_data);
static GFSCacheData* lookup_internal(GFSCache *cache, const char *pathname, FSCacheLookup lookup_type);
...
g_fscache_lookup_full(GFSCache *cache, const char *pathname, FSCacheLookup lookup_type, gboolean *found)
{
	...
	if (data->data) g_object_ref(data->data);
