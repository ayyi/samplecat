...
#include "menutrackeritemprivate.h"
...
#else
static gpointer gtk_menu_tracker_item_parent_class = NULL;

GType
gtk_menu_tracker_item_get_type (void)
{
	GType type = g_type_from_name("GtkMenuTrackerItem");
	gpointer k = g_type_class_peek(type);
	gtk_menu_tracker_item_parent_class = g_type_class_peek_parent(k);
	return type;
}
#endif
...
#if 0
static void
gtk_menu_tracker_item_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
gtk_menu_tracker_item_class_init (GtkMenuTrackerItemClass *class)
{
  ...
  gtk_menu_tracker_item_pspecs[PROP_ROLE] =
    g_param_spec_enum ("role", NULL, NULL, GTK_TYPE_MENU_TRACKER_ITEM_ROLE, GTK_MENU_TRACKER_ITEM_ROLE_NORMAL, G_PARAM_STATIC_STRINGS | G_PARAM_READABLE);
...
#else
      GParamSpec* pspec = g_object_class_find_property (G_OBJECT_GET_CLASS (self), "is-visible");
...
static void
gtk_menu_tracker_item_action_added (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name, const GVariantType *parameter_type, gboolean enabled, GVariant *state)
{
      ...
      if (GTK_DEBUG_CHECK (ACTIONS))
        {
          if (!strchr (action_name, '.'))
            dbg (0, "menutracker: action name %s doesn't look like 'app.' or 'win.'; it is unlikely to work", action_name);
        }
 ...
 */
gboolean
gtk_menu_tracker_item_get_has_link (GtkMenuTrackerItem *self, const char *link_name)
{
  GMenuModel *link;
...
#else
static gpointer gtk_menu_tracker_opener_parent_class = NULL;

GType
gtk_menu_tracker_opener_get_type (void)
{
	GType type = g_type_from_name("GtkMenuTrackerOpener");
	gpointer k = g_type_class_peek(type);
	gtk_menu_tracker_opener_parent_class = g_type_class_peek_parent(k);
	return type;
}
#endif
  ...
  if (opener->item != NULL)
    {
      ...
      gtk_action_observable_unregister_observer (item->observable, opener->submenu_action, (GtkActionObserver *)opener);

      if (AYYI_IS_ACTION_MUXER (item->observable))
        gtk_action_muxer_change_action_state (GTK_ACTION_MUXER (item->observable), opener->submenu_action, g_variant_new_boolean (FALSE));
...
void
gtk_menu_tracker_item_request_submenu_shown (GtkMenuTrackerItem *self, gboolean shown)
{
  const char *submenu_action;
  ...
  if (has_submenu_action)
    {
      if (shown)
        g_object_set_data_full (G_OBJECT (self), "submenu-opener", gtk_menu_tracker_opener_new (self, submenu_action), g_object_unref);
      else
