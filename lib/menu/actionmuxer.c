...
#include "actionmuxerprivate.h"

#include "actionobservableprivate.h"
#include "actionobserverprivate.h"
#include "widgetprivate.h"
#include "bitmaskprivate.h"
...
static guint
gtk_accels_find (GtkAccels *accels, const char *action_and_target)
{
  guint i;
...
#else
static gpointer gtk_action_muxer_parent_class = NULL;

GType
gtk_action_muxer_get_type (void)
{
	GType type = g_type_from_name("GtkActionMuxer");
	gpointer k = g_type_class_peek(type);
	gtk_action_muxer_parent_class = g_type_class_peek_parent(k);
	return type;
}
#endif
...
static GVariant *
prop_action_get_state (GtkWidget *widget, GtkWidgetAction *action)
{
  ...
  g_object_get_property (G_OBJECT (widget), action->pspec->name, &value);
											...
											#if 0
  GVariant* result = g_settings_set_mapping (&value, action->state_type, NULL);
											#endif
  g_value_unset (&value);
...
static void
prop_action_set_state (GtkWidget *widget, GtkWidgetAction *action, GVariant *state)
{
	GValue value = G_VALUE_INIT;
...
#if 0
static void
prop_action_notify (GObject *object, GParamSpec *pspec, gpointer user_data)
{
...
static gboolean
action_muxer_query_action (GtkActionMuxer *muxer, const char *action_name, gboolean *enabled, const GVariantType **parameter_type, const GVariantType **state_type, GVariant **state_hint, GVariant **state, gboolean recurse)
{
  GtkWidgetAction *action;
  Group *group;
  const char *unprefixed_name;
  ...
  if (group)
    return g_action_group_query_action (group->group, unprefixed_name, enabled, parameter_type, state_type, state_hint, state);

  if (muxer->parent && recurse)
    return gtk_action_muxer_query_action (muxer->parent, action_name, enabled, parameter_type, state_type, state_hint, state);

  return FALSE;
}

gboolean
gtk_action_muxer_query_action (GtkActionMuxer *muxer, const char *action_name, gboolean *enabled, const GVariantType **parameter_type, const GVariantType **state_type, GVariant **state_hint, GVariant **state)
{
  return action_muxer_query_action (muxer, action_name, enabled, parameter_type, state_type, state_hint, state, TRUE);
}

gboolean
gtk_action_muxer_has_action (GtkActionMuxer *muxer, const char *action_name)
{
  return action_muxer_query_action (muxer, action_name, NULL, NULL, NULL, NULL, NULL, TRUE);
}

void
gtk_action_muxer_activate_action (GtkActionMuxer *muxer, const char *action_name, GVariant *parameter)
{
  const char *unprefixed_name;
  Group *group;
...
static void
gtk_action_muxer_register_observer (GtkActionObservable *observable, const char *name, GtkActionObserver *observer)
{
  ...
  if (!is_duplicate)
    {
      if (action_muxer_query_action (muxer, name, &enabled, &parameter_type, NULL, NULL, &state, TRUE))
        {
      ...
      if (muxer->parent)
        {
          gtk_action_observable_register_observer (GTK_ACTION_OBSERVABLE (muxer->parent), name, GTK_ACTION_OBSERVER (muxer));
        }
    }
}
#endif
...
gtk_action_muxer_observer_action_added (GtkActionObserver    *observer,
                                        ...
                                        GVariant             *state)
{
  if (action_muxer_query_action (GTK_ACTION_MUXER (observer), action_name, NULL, NULL, NULL, NULL, NULL, FALSE))
    return;

  gtk_action_muxer_action_added (GTK_ACTION_MUXER (observer), action_name, parameter_type, enabled, state);
}

static void
gtk_action_muxer_observer_action_removed (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name)
{
  if (action_muxer_query_action (GTK_ACTION_MUXER (observer), action_name, NULL, NULL, NULL, NULL, NULL, FALSE))
    return;
...
static void
gtk_action_muxer_observer_action_enabled_changed (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name, gboolean enabled)
{
  if (action_muxer_query_action (GTK_ACTION_MUXER (observer), action_name, NULL, NULL, NULL, NULL, NULL, FALSE))
    return;
...
static void
gtk_action_muxer_observer_action_state_changed (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name, GVariant *state)
{
  if (action_muxer_query_action (GTK_ACTION_MUXER (observer), action_name, NULL, NULL, NULL, NULL, NULL, FALSE))
    return;
...
static void
gtk_action_muxer_observer_primary_accel_changed (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name, const char *action_and_target)
{
  gtk_action_muxer_primary_accel_changed (GTK_ACTION_MUXER (observer), action_name, action_and_target);
}
#endif
...
gtk_action_muxer_class_init (GObjectClass *class)
{
  ...
  properties[PROP_PARENT] = g_param_spec_object ("parent", NULL, NULL, GTK_TYPE_ACTION_MUXER, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_WIDGET] = g_param_spec_object ("widget", NULL, NULL, GTK_TYPE_WIDGET, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
 ...
 */
void
gtk_action_muxer_insert (GtkActionMuxer *muxer, const char *prefix, GActionGroup *action_group)
{
  Group *group;
  ...
  group->handler_ids[0] = g_signal_connect (group->group, "action-added", G_CALLBACK (gtk_action_muxer_action_added_to_group), group);
  group->handler_ids[1] = g_signal_connect (group->group, "action-removed", G_CALLBACK (gtk_action_muxer_action_removed_from_group), group);
  group->handler_ids[2] = g_signal_connect (group->group, "action-enabled-changed", G_CALLBACK (gtk_action_muxer_group_action_enabled_changed), group);
  group->handler_ids[3] = g_signal_connect (group->group, "action-state-changed", G_CALLBACK (gtk_action_muxer_group_action_state_changed), group);
}
 ...
 */
void
gtk_action_muxer_remove (GtkActionMuxer *muxer, const char *prefix)
{
  if (!muxer->groups)
    return;

  Group* group = g_hash_table_lookup (muxer->groups, prefix);
...
{
  return g_object_new (GTK_TYPE_ACTION_MUXER, "widget", widget, NULL);
}
 ...
 */
void
gtk_action_muxer_set_parent (GtkActionMuxer *muxer, GtkActionMuxer *parent)
{
...
#else
	//GParamSpec* pspec = g_object_class_find_property (G_OBJECT_GET_CLASS (muxer), "parent");
	g_object_notify_by_pspec (G_OBJECT (muxer), g_object_class_find_property (G_OBJECT_GET_CLASS (muxer), "parent"));
#endif
}

void
gtk_action_muxer_set_primary_accel (GtkActionMuxer *muxer, const char *action_and_target, const char *primary_accel)
{
  if (primary_accel)
...
const char *
gtk_action_muxer_get_primary_accel (GtkActionMuxer *muxer, const char *action_and_target)
{
   guint position = gtk_accels_find (&muxer->primary_accels, action_and_target);
...
char*
gtk_print_action_and_target (const char *action_namespace, const char *action_name, GVariant *target)
{
  GString *result;
