...
#else
static gpointer gtk_gizmo_parent_class = NULL;

GType
gtk_gizmo_get_type (void)
{
	GType type = g_type_from_name("GtkGizmo");
	gpointer k = g_type_class_peek(type);
	gtk_gizmo_parent_class = g_type_class_peek_parent(k);
	return type;
}
#endif
...
#if 0
static void
gtk_gizmo_measure (GtkWidget *widget, GtkOrientation  orientation, int for_size, int *minimum, int *natural, int *minimum_baseline, int *natural_baseline)
{
  GtkGizmo *self = GTK_GIZMO (widget);

  if (self->measure_func)
    self->measure_func (self, orientation, for_size, minimum, natural, minimum_baseline, natural_baseline);
}

static void
gtk_gizmo_size_allocate (GtkWidget *widget, int width, int height, int baseline)
{
  GtkGizmo *self = GTK_GIZMO (widget);
...
static void
gtk_gizmo_snapshot (GtkWidget *widget, GtkSnapshot *snapshot)
{
  GtkGizmo *self = GTK_GIZMO (widget);
...
static gboolean
gtk_gizmo_contains (GtkWidget *widget, double x, double y)
{
  GtkGizmo *self = GTK_GIZMO (widget);
...
static gboolean
gtk_gizmo_focus (GtkWidget *widget, GtkDirectionType direction)
{
  GtkGizmo *self = GTK_GIZMO (widget);
