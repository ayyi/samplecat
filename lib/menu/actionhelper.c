...
static void
gtk_action_helper_report_change (GtkActionHelper *helper, guint prop_id)
{
  helper->reporting++;
...
static void
gtk_action_helper_action_added (GtkActionHelper *helper, gboolean enabled, const GVariantType *parameter_type, GVariant *state, gboolean should_emit_signals)
{
	...
	helper->can_activate = (helper->target == NULL && parameter_type == NULL) || (helper->target != NULL && parameter_type != NULL && g_variant_is_of_type (helper->target, parameter_type));

	if (!helper->can_activate) {
		g_warning ("%s: action %s can't be activated due to parameter type mismatch " "(parameter type %s, target type %s)", "actionhelper", helper->action_name, parameter_type ? g_variant_type_peek_string (parameter_type) : "NULL", helper->target ? g_variant_get_type_string (helper->target) : "NULL");
		return;
	}
...
static void
gtk_action_helper_action_removed (GtkActionHelper* helper, gboolean should_emit_signals)
{
...
static void
gtk_action_helper_action_enabled_changed (GtkActionHelper *helper, gboolean enabled)
{
...
static void
gtk_action_helper_action_state_changed (GtkActionHelper *helper, GVariant *new_state)
{
	gboolean was_active;
...
static void
gtk_action_helper_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
gtk_action_helper_observer_action_added (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name, const GVariantType *parameter_type, gboolean enabled, GVariant *state)
{
  gtk_action_helper_action_added (GTK_ACTION_HELPER (observer), enabled, parameter_type, state, TRUE);
}

static void
gtk_action_helper_observer_action_enabled_changed (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name, gboolean enabled)
{
  gtk_action_helper_action_enabled_changed (GTK_ACTION_HELPER (observer), enabled);
}

static void
gtk_action_helper_observer_action_state_changed (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name, GVariant *state)
{
	gtk_action_helper_action_state_changed (GTK_ACTION_HELPER (observer), state);
}

static void
gtk_action_helper_observer_action_removed (GtkActionObserver *observer, GtkActionObservable *observable, const char *action_name)
{
...
gtk_action_helper_class_init (GtkActionHelperClass *class)
{
	...
	gtk_action_helper_pspecs[PROP_ENABLED] = g_param_spec_boolean ("enabled", NULL, NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
	gtk_action_helper_pspecs[PROP_ACTIVE] = g_param_spec_boolean ("active", NULL, NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
	gtk_action_helper_pspecs[PROP_ROLE] = g_param_spec_enum ("role", NULL, NULL, GTK_TYPE_BUTTON_ROLE, GTK_BUTTON_ROLE_NORMAL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
...
gtk_action_helper_new (GtkActionable* widget)
{
	...
	GtkActionHelper* helper = g_object_new (GTK_TYPE_ACTION_HELPER, NULL);

	helper->widget = GTK_WIDGET (widget);
	helper->enabled = gtk_widget_get_sensitive (GTK_WIDGET (helper->widget));

	GParamSpec* pspec = g_object_class_find_property (G_OBJECT_GET_CLASS (helper->widget), "active");
	if (pspec && G_PARAM_SPEC_VALUE_TYPE (pspec) == G_TYPE_BOOLEAN)
...
void
gtk_action_helper_set_action_name (GtkActionHelper *helper, const char *action_name)
{
  ...
  if (helper->action_name) {
...
#ifdef GTK4_TODO
      gtk_action_observable_unregister_observer (GTK_ACTION_OBSERVABLE (helper->action_context), helper->action_name, GTK_ACTION_OBSERVER (helper));
#endif
	...
	if (action_name) {
		...
		if (gtk_action_muxer_query_action (helper->action_context, helper->action_name, &enabled, &parameter_type, NULL, NULL, &state)) {
#else
		if (false) {
#endif
 ...
 */
void
gtk_action_helper_set_action_target_value (GtkActionHelper *helper, GVariant *target_value)
{
...
#ifdef GTK4_TODO
	gtk_action_muxer_activate_action (helper->action_context, helper->action_name, helper->target);
#endif
}
