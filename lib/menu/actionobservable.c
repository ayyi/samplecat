...
#else
GType
gtk_action_observable_get_type (void)
{
	GType type = g_type_from_name("GtkActionObservable");
	return type;
}
#endif
 ...
 */
void
gtk_action_observable_register_observer (GtkActionObservable *observable, const char *action_name, GtkActionObserver *observer)
{
  g_return_if_fail (GTK_IS_ACTION_OBSERVABLE (observable));

  GTK_ACTION_OBSERVABLE_GET_IFACE (observable)->register_observer (observable, action_name, observer);
}
 ...
 */
void
gtk_action_observable_unregister_observer (GtkActionObservable *observable, const char *action_name, GtkActionObserver *observer)
{
  g_return_if_fail (GTK_IS_ACTION_OBSERVABLE (observable));

  GTK_ACTION_OBSERVABLE_GET_IFACE (observable)->unregister_observer (observable, action_name, observer);
}
