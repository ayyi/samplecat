...
#else
static gpointer gtk_model_button_parent_class = NULL;

GType
gtk_model_button_get_type (void)
{
	GType type = g_type_from_name("GtkModelButton");
	gtk_model_button_parent_class = g_type_class_peek_parent(g_type_class_peek(type));
	return type;
}
#endif
  ...
  if (g_once_init_enter (&gtk_button_role_type))
    {
      ...
      type = g_enum_register_static ("GtkButtonRole", values);
...
update_at_context (GtkModelButton *button)
{
  ...
  gboolean was_realized = gtk_at_context_is_realized (context);
...
update_node_name (GtkModelButton *self)
{
  ...
  if (self->iconic)
	...
	if (start_name && !self->start_indicator) {
		self->start_indicator = gtk_image_new_from_icon_name (start_name);
	...
	if (end_name && !self->end_indicator) {
		self->end_indicator = gtk_image_new_from_icon_name (end_name);
...
static void
gtk_model_button_set_iconic (GtkModelButton *self, gboolean iconic)
{
#ifdef GTK4_TODO
...
#if 0
static void
gtk_model_button_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
gtk_model_button_clicked (GtkModelButton *self)
{
	if (self->menu_name != NULL) {
		...
		AyyiPopoverMenu* menu = (AyyiPopoverMenu*)gtk_widget_get_ancestor (GTK_WIDGET (self), GTK_TYPE_POPOVER_MENU);
		GtkWidget* submenu = self->popover;

		gtk_popover_popup (GTK_POPOVER (submenu));
		ayyi_popover_menu_set_open_submenu (menu, submenu);
		ayyi_popover_menu_set_parent_menu (AYYI_POPOVER_MENU (submenu), GTK_WIDGET (menu));

		gtk_accessible_update_state (GTK_ACCESSIBLE (self), GTK_ACCESSIBLE_STATE_EXPANDED, TRUE, -1);
	} else if (!self->keep_open) {
		GtkWidget* popover = gtk_widget_get_ancestor (GTK_WIDGET (self), GTK_TYPE_POPOVER);
		if (popover)
			gtk_popover_popdown (GTK_POPOVER (popover));

		gtk_accessible_update_state (GTK_ACCESSIBLE (self), GTK_ACCESSIBLE_STATE_EXPANDED, FALSE, -1);
	}
...
static gboolean
toggle_cb (GtkWidget *widget, GVariant *args, gpointer user_data)
{
...
static gboolean
gtk_model_button_focus (GtkWidget *widget, GtkDirectionType direction)
{
	GtkModelButton *button = GTK_MODEL_BUTTON (widget);

	if (gtk_widget_is_focus (widget)) {
		if (direction == GTK_DIR_LEFT && button->role == GTK_BUTTON_ROLE_TITLE && button->menu_name != NULL) {
			switch_menu (button);
			return TRUE;
		} else if (direction == GTK_DIR_RIGHT && button->role == GTK_BUTTON_ROLE_NORMAL && button->menu_name != NULL) {
			switch_menu (button);
			return TRUE;
        } else if (direction == GTK_DIR_RIGHT && button->role == GTK_BUTTON_ROLE_NORMAL && button->popover != NULL) {
			AyyiPopoverMenu* menu = AYYI_POPOVER_MENU (gtk_widget_get_ancestor (GTK_WIDGET (button), GTK_TYPE_POPOVER_MENU));
...
gtk_model_button_class_init (GtkModelButtonClass *class)
{
 ...
 properties[PROP_POPOVER] =
  ...
  properties[PROP_ICONIC] =
	 ...
	 */
	properties[PROP_INDICATOR_SIZE_GROUP] = g_param_spec_object ("indicator-size-group", NULL, NULL, GTK_TYPE_SIZE_GROUP, G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);
	properties[PROP_ACCEL] = g_param_spec_string ("accel", NULL, NULL, NULL, G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);
	g_object_class_install_properties (object_class, LAST_PROP, properties);

	g_object_class_override_property (object_class, PROP_ACTION_NAME, "action-name");
	g_object_class_override_property (object_class, PROP_ACTION_TARGET, "action-target");

	signals[SIGNAL_CLICKED] = g_signal_new ("clicked", G_OBJECT_CLASS_TYPE (object_class), G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION, G_STRUCT_OFFSET (GtkModelButtonClass, clicked), NULL, NULL, NULL, G_TYPE_NONE, 0);
	...
	for (int i = 0; i < G_N_ELEMENTS (activate_keyvals); i++) {
      GtkShortcut* shortcut = gtk_shortcut_new (gtk_keyval_trigger_new (activate_keyvals[i], 0), g_object_ref (action));
	...
	for (int i = 0; i < G_N_ELEMENTS (toggle_keyvals); i++) {
		GtkShortcut* shortcut = gtk_shortcut_new (gtk_keyval_trigger_new (toggle_keyvals[i], 0), g_object_ref (action));
		gtk_widget_class_add_shortcut (widget_class, shortcut);
		g_object_unref (shortcut);
	}
	g_object_unref (action);
...
open_submenu (gpointer data)
{
	...
	GtkPopover* popover = (GtkPopover*)gtk_widget_get_ancestor (GTK_WIDGET (button), GTK_TYPE_POPOVER);

	if (GTK_IS_POPOVER_MENU (popover)) {
		ayyi_popover_menu_set_active_item (AYYI_POPOVER_MENU (popover), GTK_WIDGET (button));

		if (button->popover) {
          GtkWidget *submenu = button->popover;

          if (AYYI_POPOVER_MENU (popover)->open_submenu != submenu)
            ayyi_popover_menu_close_submenus (AYYI_POPOVER_MENU (popover));

          gtk_popover_popup (GTK_POPOVER (submenu));
          ayyi_popover_menu_set_open_submenu (AYYI_POPOVER_MENU (popover), submenu);
          ayyi_popover_menu_set_parent_menu (AYYI_POPOVER_MENU (submenu), GTK_WIDGET (popover));
        }
    }
...
#if 0
static void
pointer_cb (GObject* object, GParamSpec* pspec, gpointer data)
{
	GtkWidget *target = GTK_WIDGET (data);
	gboolean contains = gtk_event_controller_motion_contains_pointer (GTK_EVENT_CONTROLLER_MOTION (object));
	GtkWidget* popover = gtk_widget_get_ancestor (target, GTK_TYPE_POPOVER_MENU);

	if (contains) {
		if (popover) {
			if (AYYI_POPOVER_MENU (popover)->open_submenu != NULL)
		...
		stop_open (button);
		if (popover)
			ayyi_popover_menu_set_active_item (AYYI_POPOVER_MENU (popover), NULL);
	}
}

static void
motion_cb (GtkEventController* controller, double x, double y, gpointer data)
{
	start_open (GTK_MODEL_BUTTON (data));
}

static void
focus_in_cb (GtkEventController* controller, gpointer data)
{
	GtkWidget* target = gtk_event_controller_get_widget (controller);
	GtkWidget* popover = gtk_widget_get_ancestor (target, GTK_TYPE_POPOVER_MENU);

	if (popover)
		ayyi_popover_menu_set_active_item (AYYI_POPOVER_MENU (popover), target);
}

static void
gesture_pressed (GtkGestureClick* gesture, guint n_press, double x, double y, GtkWidget *widget)
{
...
static void
gesture_unpaired_release (GtkGestureClick  *gesture, double x, double y, guint button, GdkEventSequence *sequence, GtkWidget *widget)
{
  if (gtk_widget_contains (widget, x, y))
    g_signal_emit (widget, signals[SIGNAL_CLICKED], 0);
}
