...
#include "menutrackerprivate.h"
#include "gizmoprivate.h"
#include "menusectionboxprivate.h"
#include "widgetprivate.h"
#include "popovermenuprivate.h"

typedef GtkBoxClass AyyiMenuSectionBoxClass;

struct _AyyiMenuSectionBox
{
  GtkBox               parent_instance;

  AyyiMenuSectionBox*  toplevel;
...
G_DEFINE_TYPE (AyyiMenuSectionBox, ayyi_menu_section_box, GTK_TYPE_BOX)

static void       ayyi_menu_section_box_sync_separators (AyyiMenuSectionBox*, MenuData*);
static void       ayyi_menu_section_box_new_submenu     (GtkMenuTrackerItem*, AyyiMenuSectionBox* toplevel, GtkWidget* focus, const char* name);
static GtkWidget* ayyi_menu_section_box_new_section     (GtkMenuTrackerItem*, AyyiMenuSectionBox* parent);
 ...
 */
static void
ayyi_menu_section_box_sync_separators (AyyiMenuSectionBox* box, MenuData* data)
{
	...
	for (child = gtk_widget_get_first_child (GTK_WIDGET (box->item_box)); child != NULL; child = gtk_widget_get_next_sibling (child)) {
		if (AYYI_IS_MENU_SECTION_BOX (child))
			ayyi_menu_section_box_sync_separators (AYYI_MENU_SECTION_BOX (child), data);
		else
			data->n_items++;
	}

	gboolean is_not_empty_item = (data->n_items > n_items_before);
	...
	gboolean has_separator = gtk_widget_get_parent (box->separator) != NULL;
	gboolean has_label = !GTK_IS_SEPARATOR (box->separator);

	gboolean separator_condition = has_label
		? TRUE
		: n_items_before > 0 && box->depth <= 1 && !previous_section_is_iconic && !box->iconic;
...
{
  AyyiMenuSectionBox *box = user_data;
  MenuData data;

  data.n_items = 0;
  data.previous_is_iconic = FALSE;
  ayyi_menu_section_box_sync_separators (box, &data);
...
static void
gtk_menu_section_box_schedule_separator_sync (AyyiMenuSectionBox *box)
{
	box = box->toplevel;

	if (!box->separator_sync_idle) {
		box->separator_sync_idle = g_idle_add_full (G_PRIORITY_DEFAULT, /* before menu is drawn... */ gtk_menu_section_box_handle_sync_separators, box, NULL);
		gdk_source_set_static_name_by_id (box->separator_sync_idle, "[gtk] menu section box handle sync separators");
	}
}

static void
gtk_popover_item_activate (GtkWidget *button, gpointer user_data)
{
...
static void
gtk_menu_section_box_remove_func (int position, gpointer user_data)
{
  AyyiMenuSectionBox *box = user_data;
...
static gboolean
get_ancestors (GtkWidget *widget, GType widget_type, GtkWidget **ancestor, GtkWidget **below)
{
  GtkWidget *a, *b;
...
static void
close_submenu (GtkWidget *button, gpointer data)
{
...
static void
open_submenu (GtkWidget *button, gpointer data)
{
  GtkMenuTrackerItem *item = data;
  GtkWidget *focus;
...
static void
submenu_shown (GtkPopoverMenu *popover, GtkMenuTrackerItem *item)
{
  if (gtk_menu_tracker_item_get_should_request_show (item))
    gtk_menu_tracker_item_request_submenu_shown (item, TRUE);
}
...
static void
submenu_hidden (GtkPopoverMenu *popover, GtkMenuTrackerItem *item)
{
  if (gtk_menu_tracker_item_get_should_request_show (item))
    gtk_menu_tracker_item_request_submenu_shown (item, FALSE);
}
 ...
 */
static gboolean
custom_widget_focus (GtkGizmo* gizmo, GtkDirectionType direction)
{
	return gtk_widget_focus_move (GTK_WIDGET (gizmo), direction);
}
...
static void
gtk_menu_section_box_insert_func (GtkMenuTrackerItem* item, int position, gpointer user_data)
{
	AyyiMenuSectionBox *box = user_data;
	GtkWidget *widget;

	if (gtk_menu_tracker_item_get_is_separator (item)) {
		widget = ayyi_menu_section_box_new_section (item, box);

	} else if (gtk_menu_tracker_item_get_has_link (item, G_MENU_LINK_SUBMENU)) {
		if (box->flags & GTK_POPOVER_MENU_NESTED) {
			GMenuModel* model = _gtk_menu_tracker_item_get_link (item, G_MENU_LINK_SUBMENU);

			GtkWidget* submenu = gtk_popover_menu_new_from_model_full (model, box->flags);
			gtk_popover_set_has_arrow (GTK_POPOVER (submenu), FALSE);
			gtk_widget_set_valign (submenu, GTK_ALIGN_START);

			widget = g_object_new (GTK_TYPE_MODEL_BUTTON, "popover", submenu, "indicator-size-group", box->indicators, NULL);
			...
			widget = g_object_new (GTK_TYPE_MODEL_BUTTON, "menu-name", gtk_menu_tracker_item_get_label (item), "indicator-size-group", box->indicators, NULL);
			g_object_bind_property (item, "label", widget, "text", G_BINDING_SYNC_CREATE);
			g_object_bind_property (item, "icon", widget, "icon", G_BINDING_SYNC_CREATE);
			g_object_bind_property (item, "use-markup", widget, "use-markup", G_BINDING_SYNC_CREATE);
			g_object_bind_property (item, "sensitive", widget, "sensitive", G_BINDING_SYNC_CREATE);
			...
			ayyi_menu_section_box_new_submenu (item, box->toplevel, widget, name);
			g_free (name);
		}
		...
		widget = g_object_new (GTK_TYPE_MODEL_BUTTON, "indicator-size-group", box->indicators, NULL);
		g_object_bind_property (item, "label", widget, "text", G_BINDING_SYNC_CREATE);
...
static void
ayyi_menu_section_box_init (AyyiMenuSectionBox *box)
{
...
{
	AyyiMenuSectionBox *box = AYYI_MENU_SECTION_BOX (object);
	...
	G_OBJECT_CLASS (ayyi_menu_section_box_parent_class)->dispose (object);
}

static void
ayyi_menu_section_box_class_init (AyyiMenuSectionBoxClass *class)
{
	G_OBJECT_CLASS (class)->dispose = gtk_menu_section_box_dispose;
}

static void
update_popover_position_cb (GObject *source, GParamSpec *spec, gpointer *user_data)
{
  GtkPopover *popover = GTK_POPOVER (source);
  AyyiMenuSectionBox *box = AYYI_MENU_SECTION_BOX (user_data);
  GtkWidget *w;
...
void
ayyi_menu_section_box_new_toplevel (AyyiPopoverMenu* popover, GMenuModel* model, GtkPopoverMenuFlags flags)
{
	AyyiMenuSectionBox* box = g_object_new (AYYI_TYPE_MENU_SECTION_BOX, NULL);
	box->indicators = gtk_size_group_new (GTK_SIZE_GROUP_HORIZONTAL);
	box->custom_slots = g_hash_table_new (g_str_hash, g_str_equal);
	box->flags = flags;

	ayyi_popover_menu_add_submenu (popover, GTK_WIDGET (box), "main");

	GtkActionObservable* action_muxer = GTK_ACTION_OBSERVABLE (_gtk_widget_get_action_muxer (GTK_WIDGET (box), TRUE));
	box->tracker = gtk_menu_tracker_new (action_muxer, model, TRUE, FALSE, FALSE, NULL, gtk_menu_section_box_insert_func, gtk_menu_section_box_remove_func, box);

	g_signal_connect (G_OBJECT (popover), "notify::position", G_CALLBACK (update_popover_position_cb), box);
}

static void
ayyi_menu_section_box_new_submenu (GtkMenuTrackerItem* item, AyyiMenuSectionBox* toplevel, GtkWidget* focus, const char* name)
{
	AyyiMenuSectionBox* box = g_object_new (AYYI_TYPE_MENU_SECTION_BOX, NULL);
	box->indicators = gtk_size_group_new (GTK_SIZE_GROUP_HORIZONTAL);
	...
	GtkWidget* button = g_object_new (GTK_TYPE_MODEL_BUTTON, "menu-name", name, "role", GTK_BUTTON_ROLE_TITLE, NULL);
	...
	gtk_stack_add_named (GTK_STACK (gtk_widget_get_ancestor (GTK_WIDGET (toplevel), GTK_TYPE_STACK)), GTK_WIDGET (box), gtk_menu_tracker_item_get_label (item));

	box->tracker = gtk_menu_tracker_new_for_item_link (item, G_MENU_LINK_SUBMENU, FALSE, FALSE, gtk_menu_section_box_insert_func, gtk_menu_section_box_remove_func, box);
}

static GtkWidget*
ayyi_menu_section_box_new_section (GtkMenuTrackerItem* item, AyyiMenuSectionBox* parent)
{
	AyyiMenuSectionBox* box = g_object_new (AYYI_TYPE_MENU_SECTION_BOX, NULL);
	...
	const char* label = gtk_menu_tracker_item_get_label (item);
	const char* hint = gtk_menu_tracker_item_get_display_hint (item);
	const char* text_direction = gtk_menu_tracker_item_get_text_direction (item);
	...
	if (hint && g_str_equal (hint, "horizontal-buttons")) {
		...
		GtkWidget* spacer = gtk_gizmo_new ("none", NULL, NULL, NULL,NULL, NULL, NULL);
  ...
  box->tracker = gtk_menu_tracker_new_for_item_link (item, G_MENU_LINK_SECTION, FALSE, FALSE, gtk_menu_section_box_insert_func, gtk_menu_section_box_remove_func, box);

  return GTK_WIDGET (box);
}

gboolean
ayyi_menu_section_box_add_custom (AyyiPopoverMenu *popover, GtkWidget *child, const char *id)
{
	GtkWidget* stack = ayyi_popover_menu_get_stack (popover);
	AyyiMenuSectionBox* box = AYYI_MENU_SECTION_BOX (gtk_stack_get_child_by_name (GTK_STACK (stack), "main"));
	if (box == NULL)
		return FALSE;

	GtkWidget* slot = (GtkWidget *)g_hash_table_lookup (box->custom_slots, id);
...
gboolean
ayyi_menu_section_box_remove_custom (AyyiPopoverMenu *popover, GtkWidget *child)
{
	GtkWidget* stack = ayyi_popover_menu_get_stack (popover);
	AyyiMenuSectionBox* box = AYYI_MENU_SECTION_BOX (gtk_stack_get_child_by_name (GTK_STACK (stack), "main"));
	if (box == NULL)
	...
	const char* id = (const char *) g_object_get_data (G_OBJECT (parent), "slot-id");
